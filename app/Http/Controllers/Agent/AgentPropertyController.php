...
class AgentPropertyController extends Controller
{
    ...
    public function AgentAddProperty()
    {
        ...
        $id = Auth::user()->id;
        $propertylimit = User::where('role', 'agent')->where('id', $id)->first();
        $pcount = $propertylimit->credit;
        // dd($pcount);
        if ($pcount == 1 || $pcount == 7) {

            $notification = array(
                'message' => 'your property limit has reached!',
                'alert-type' => 'success'
            );
    ...
    public function StoreProfessionalPlan(Request $request)
    {
        ...
        $notification = array(
            'message' => 'pro plan added successfully',
            'alert-type' => 'success'
        );
        return redirect()->route('agent.all.property')->with($notification);
    } //end
